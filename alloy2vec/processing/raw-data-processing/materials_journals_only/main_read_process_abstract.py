...
def print_meta_data_citations(citations,dois_unique_years,citations_unique_doi_years,filename,years):
  for index in range(len(years)):
    ...
    write(citations_unique_doi_years[index],"citations/"+filename+'_'+years[index]+'_citations_doi_unique.dat')
    write(dois_unique_years[index],"citations/"+filename+'_'+years[index]+'_dois_unique.dat')
  return None
if __name__=="__main__":
  ...
  if sys.argv[1]=="time":
    ...
    corpus,dois,dois_unique_years,corpus_unique_doi_years=extract_abstract_time(json_dir,years,journals)
